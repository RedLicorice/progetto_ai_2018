...
public class ArchiveService {
    ...
    @Transactional
    public void deleteArchive(String archiveId) throws MeasuresNotFoundException, ArchiveNotFoundException {
        Optional<Archive> archive = archiveRepo.findById(archiveId);
        if(!archive.isPresent())
            throw new ArchiveNotFoundException(archiveId);
        archive.get().setDeleted(true);
        archiveRepo.save(archive.get());

        Optional<List<Measure>> measures = measureRepo.findAllByArchive(archiveId);
